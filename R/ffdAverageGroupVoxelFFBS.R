...
ffdAverageGroupVoxelFFBS <- function(posi.ffd, DatabaseGroup, covariates, m0, Cova, delta, S0, n0, N1, Nsimu1, r1, Cutpos){
  ...
  if(any(series.group[,1]==0)){return( list(EvidenceLTT = rep(NA, dim(covariates)[2]), 
                                            EvidenceMargin = rep(NA, dim(covariates)[2])) )}else{
                                              ...
                                              for(j in 1:Ngroup){
                                                ...
                                                OnlineThetaM = OnlineThetaM + res$Online_theta[-(1:Cutpos),,]
                                                OnlineThetaL = OnlineThetaL + res$Online_theta_mean[-(1:Cutpos),,]
                                              }
